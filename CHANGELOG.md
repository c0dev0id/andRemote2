...
### Added
- **Accessibility service for foreground detection**: replaces Usage Access permission. `RemoteAccessibilityService` listens for `TYPE_WINDOW_STATE_CHANGED` events — the same mechanism used by wireddevices — giving instant, accurate foreground app detection with no polling lag. Enable via the new button in Overlay Settings.
